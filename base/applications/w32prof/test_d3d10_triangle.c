...
void
W32Prof_Test_D3D10Triangle(const ProfilerConfig* cfg)
{
    ...
    if (!hRender)
    {
        ResultsPrint(TEXT("D3D10 Triangle: failed to create render child window"));
        return;
    }
    ...
    {
        ResultsPrint(TEXT("D3D10 Triangle: CreateDXGIFactory failed: 0x%08lx"), (ULONG)hr);
    ...
    {
        ResultsPrint(TEXT("D3D10 Triangle: EnumAdapters failed: 0x%08lx"), (ULONG)hr);
    ...
    if (FAILED(hr))
    {
        ...
        if (FAILED(hr))
        {
            ResultsPrint(TEXT("D3D10 Triangle: D3D10CreateDeviceAndSwapChain failed: 0x%08lx"), (ULONG)hr);
...

    {
        ...
        {
            ResultsPrint(TEXT("D3D10 Triangle: CreateRenderTargetView failed: 0x%08lx"), (ULONG)hr);
    ...
    if (FAILED(hr))
    {
        if (errorBlob)
        {
            ResultsPrint(TEXT("D3D10 Triangle: VS compile error: %s"), (char*)ID3D10Blob_GetBufferPointer(errorBlob));
            ID3D10Blob_Release(errorBlob);
        }
        ResultsPrint(TEXT("D3D10 Triangle: D3DCompile (VS) failed: 0x%08lx"), (ULONG)hr);
    ...
    if (FAILED(hr))
    {
        if (errorBlob)
        {
            ResultsPrint(TEXT("D3D10 Triangle: PS compile error: %s"), (char*)ID3D10Blob_GetBufferPointer(errorBlob));
            ID3D10Blob_Release(errorBlob);
        }
        ResultsPrint(TEXT("D3D10 Triangle: D3DCompile (PS) failed: 0x%08lx"), (ULONG)hr);
    ...
    {
        ResultsPrint(TEXT("D3D10 Triangle: CreateVertexShader failed: 0x%08lx"), (ULONG)hr);
    ...
    {
        ResultsPrint(TEXT("D3D10 Triangle: CreatePixelShader failed: 0x%08lx"), (ULONG)hr);
...

    {
        ...
        {
            ResultsPrint(TEXT("D3D10 Triangle: CreateInputLayout failed: 0x%08lx"), (ULONG)hr);
...

    {
        ...
        {
            ResultsPrint(TEXT("D3D10 Triangle: CreateBuffer failed: 0x%08lx"), (ULONG)hr);
    ...
    while (1)
    {
        ...
        if (FAILED(hr))
        {
            ResultsPrint(TEXT("D3D10 Triangle: Present failed: 0x%08lx"), (ULONG)hr);
            break;
        }

        i++;
        W32Prof_FpsMaybeReport(cfg, &fps, i, qf.QuadPart, TEXT("D3D10 Triangle"));
    }

    QueryPerformanceCounter(&q1);

    ResultsPrint(TEXT("D3D10 Triangle: %lu frames in %.3f ms (%.2f fps)"),
                 (ULONG)i,
